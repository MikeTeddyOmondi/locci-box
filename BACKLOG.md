...
- [x] **MCP Server — Setup & Verification**
  - Server code at `src/mcp/server.ts` — pure HTTP client over REST API, tools: `run_sandbox`, `get_sandbox_status`, `stop_sandbox`
  - Dev scripts: `pnpm mcp:dev` (tsx), `pnpm mcp:inspect` (MCP Inspector UI)
  - **Transports:**
    - **stdio** (default) — for Claude Desktop / Cursor local connections; `MCP_ENABLED=true`
    - **Streamable HTTP** — `MCP_HTTP_ENABLED=true`, `MCP_HTTP_PORT=3001`; exposes `POST /mcp` (stateless, one Server+transport per request); for remote/API clients
    - Both transports run simultaneously when both env vars are set
  - **CLI integration** (`@locci/box` package, published to npm):
    - `loccibox mcp start` — runs the MCP stdio server in-process using your active profile's API URL + key; self-contained, no separate API process needed
    - `loccibox mcp config` — prints ready-to-paste config for Claude Desktop, Cursor, or any MCP client
  - **Client config** (also generated by `loccibox mcp config`):
    ```json
    // If loccibox is installed globally (npm i -g @locci/box):
    { "mcpServers": { "locci-box": { "command": "loccibox", "args": ["mcp", "start"], "env": { "LOCCIBOX_API_URL": "...", "LOCCIBOX_API_KEY": "lbk_live_..." } } } }

    // Or via npx (no install required):
    { "mcpServers": { "locci-box": { "command": "npx", "args": ["-y", "@locci/box", "mcp", "start"], "env": { "LOCCIBOX_API_URL": "...", "LOCCIBOX_API_KEY": "lbk_live_..." } } } }
    ```
