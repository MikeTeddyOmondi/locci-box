...
function Block({ code, lang }: { code: string; lang?: string }) {
  return (
    <div className="relative group rounded-xl border hero-border bg-white overflow-hidden">
      {lang && (
        <div className="absolute top-2 left-3 text-[10px] uppercase tracking-wider hero-text-muted font-semibold">
          {lang}
        </div>
      )}
      <Button
      ...
      <pre className="overflow-auto p-4 pt-7 text-xs font-mono leading-relaxed hero-text">
        <code>{code}</code>
      </pre>
    </div>
  );
...
function Page() {
  ...
  const onNewFolder = () => {
    ...
    if (!name?.trim()) return;
    setItems((s) => [
      { id: `fd-${Date.now()}`, name: name.trim(), kind: "folder", createdAt: "just now" },
      ...s,
    ]);
            ...
            {sections.map((s) => {
              ...
              return (
                ...
                >
                  <span
                    className={cn(
                      "w-7 h-7 rounded-lg flex items-center justify-center transition-transform group-hover:scale-110 shrink-0",
                      s.gradient,
                    )}
                  >
        ...
        <div className="hero-white p-5 sm:p-8 lg:p-12 space-y-10 sm:space-y-14 max-w-none min-w-0">
          <section id="my-docs" className="space-y-4">
            <div className="flex flex-wrap items-end justify-between gap-3">
              <div>
                <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight hero-text">
                  My Documents
                </h1>
                <p className="hero-text-muted text-sm mt-1">
                  Upload, organize, and access your sandbox docs.
                </p>
              </div>
            ...
            <div className="rounded-xl border hero-border overflow-hidden bg-white">
              ...
              {filtered.length === 0 ? (
                <div className="p-6 text-center text-sm hero-text-muted">
                ...
                filtered.map((it) => (
                  <div
                    key={it.id}
                    className="grid grid-cols-[1fr_120px_140px_40px] items-center px-4 py-2.5 text-sm hero-text border-b last:border-b-0 hero-divider hover:bg-slate-50"
                  >
                    ...
                    <div className="hero-text-muted text-xs">
                      {it.kind === "folder" ? "—" : formatSize(it.size)}
                    </div>
          ...
          <section id="shared" className="space-y-4">
            <div className="flex items-end justify-between gap-3 flex-wrap">
              <div>
                ...
                <p className="hero-text-muted text-sm mt-1">
                  Documents shared across your workspace.
                </p>
              </div>
            </div>
            <div className="rounded-xl border hero-border overflow-hidden bg-white">
              {sharedItems.map((it) => (
                <div
                  key={it.id}
                  className="flex items-center gap-3 px-4 py-2.5 text-sm hero-text border-b last:border-b-0 hero-divider hover:bg-slate-50"
                >
                  {it.kind === "folder" ? (
                    <Folder className="w-4 h-4 text-amber-500 shrink-0" />
                  ) : (
  ...
  "timeout": 30
            ...
            <Block
              lang="bash"
              code={`curl https://api.sandboxapi.com/api/sandbox/sbox_abc123/status \\
  -H "Authorization: Bearer YOUR_API_KEY"`}
            ...
            <Block
              lang="bash"
              code={`curl -X DELETE https://api.sandboxapi.com/api/sandbox/sbox_abc123 \\
  -H "Authorization: Bearer YOUR_API_KEY"`}
            />
          </section>
          ...
          <section id="auth" className="space-y-4">
            ...
            <p className="text-sm hero-text-muted">
              Default 100 requests per minute. Exceeding it returns 429 with a{" "}
              <code className="font-mono text-xs px-1 py-0.5 bg-slate-100 rounded">
                retry_after
              </code>{" "}
              field.
            </p>
            ...
            <ul className="text-sm hero-text-muted space-y-1 list-disc pl-5">
              <li>
                <span className="font-mono">400</span> — Bad request (invalid language, missing
                fields)
              </li>
              <li>
                <span className="font-mono">401</span> — Missing or invalid API key
              </li>
              <li>
                <span className="font-mono">429</span> — Rate limited
              </li>
              <li>
                <span className="font-mono">500</span> — Internal sandbox error
              </li>
            </ul>
          </section>

          <section id="dual" className="space-y-4">
            <h1 className="text-3xl font-extrabold tracking-tight hero-text">
              Dual Sandbox Testing
            </h1>
            <p className="hero-text-muted">
              Two isolated microVMs working in tandem — one for source, one for tests.
            </p>
            <ol className="text-sm hero-text-muted space-y-2 list-decimal pl-5">
              ...
              <li>
                Press{" "}
                <kbd className="px-1.5 py-0.5 rounded border hero-border bg-slate-50 text-xs hero-text">
                  Ctrl/Cmd+Enter
                </kbd>{" "}
                or click Run Tests
              </li>
...
console.log(data.data.stdout);`}
            ...
            <Block
              lang="bash"
              code={`curl -X POST https://api.sandboxapi.com/api/sandbox/run \\
